...
//byte rule = ( LEFT_MSK | RIGHT2_MSK | RIGHT_MSK | MIDDLE_MSK );
//byte rule = (LEFT_MSK | LEFT_MSK | RIGHT_MSK);
...
uint32_t getPixelColor() {
  ...
  uint16_t g_color = 0;
  g_color <<= 8;    
  g_color |= random(0xFF); //Give me a number from 0 to 0xFF
